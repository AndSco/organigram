...
app.get("/api/employees", async function(req, res, next) {
  try {
    ...
    const lastUpdate = await Employee.findOne(
      {},
      {},
      { sort: { updatedAt: -1 } }
    );
    ...
    return res
      .status(200)
      .json({ employees: allEmployees, lastUpdate: lastUpdate.updatedAt });
  } catch (err) {
    return next(err);
  }
});
...
app.put("/api/employees/:employeeId", async function(req, res, next) {
  try {
    ...
    await Employee.findByIdAndUpdate(req.params.employeeId, editedEmployee, {
      new: true
    });
