...
const AddForm = props => {
	...
	const handleSubmit = (e) => {
		...
		managePicture()
			.then(res => {
				...
				const newEmployee = {designation, name, officePhone, mobile, email, team, imageUrl: res.data.url};
				axios.post(endPointData, newEmployee)
	...
	const handleUpdate = (e, employeeId) => {
		...
		if (needNewPicture) {
			...
			managePicture()
				.then(res => {
					const updatedEmployee = {designation, name, officePhone, mobile, email, team, imageUrl: res.data.url};
					console.log("UPDATED EMPLOYEE FROM NEEDpic", updatedEmployee);
					axios.put(editEndPoint, updatedEmployee)
						.then(res => console.log(res))
						.then(() => {
							props.closeForm();
			...
			const updatedEmployee = {designation, name, officePhone, mobile, email, team, imageUrl: props.staffMember.imageUrl};
			axios.put(editEndPoint, updatedEmployee)
				.then(res => console.log(res))
				.then(() => {
						props.closeForm();
	...
	return (
		<div className="form">
			<FontAwesomeIcon icon="window-close" onClick={ props.closeForm} size="lg" />
			...
			<form onSubmit={props.action === "edit" ? (e) => handleUpdate(e, props.staffMember._id) : (e) => handleSubmit(e)}>
				...
				<input type="text" name="name" placeholder="name" value={name} onChange={(e) => handleChange(e, "name")} />
				<input type="text" name="officePhone" placeholder="office phone" value={officePhone} onChange={(e) => handleChange(e, "officePhone")} />
				<input type="text" name="mobile" placeholder="office mobile (if any)" value={mobile} onChange={(e) => handleChange(e, "mobile")} />
				<input type="text" name="email" placeholder="email" value={email} onChange={(e) => handleChange(e, "email")} />
				{ props.action === "edit" && !needNewTeam 
					?
					<div className="add-team">
						<p>Team: <strong className="team-designation">{props.staffMember.team}</strong></p> 
				...
				{ props.action === "edit" && !needNewPicture && (
					<div className="change-image">
						<p>Profile picture: </p>
						<img src={props.staffMember.imageUrl} width="40" height="40" onClick={() => setNeedNewPicture(true)} alt={props.staffMember.name} />
						<FontAwesomeIcon icon="edit" onClick={() => setNeedNewPicture(true)} />
					</div>	
				) }
